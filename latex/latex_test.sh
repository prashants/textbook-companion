...
echo \\begin{description}>>$CURDIR/TEX >> $CURDIR/TEX
echo \\item[Exa]  Example \(Solved example\)>>$CURDIR/TEX >>$CURDIR/TEX
...
if [ $col7 =  D ]
then
echo check Appendix \\ref{AP:$col5} for dependency \$$col5\$ >> $CURDIR/TEX
echo \\\\ >>$CURDIR/TEX
fi
...
echo $col6 > $CURDIR/Figure_files
	if  grep -c ".jpg\|.JPEG\|.jpeg\|.png" $CURDIR/Figure_files 
	then
		 echo \\curlable{Fig~$col3} >> $CURDIR/TEX
		 echo \\begin{figure} >> $CURDIR/TEX
		 echo \\includegraphics{../$col6}  >> $CURDIR/TEX
...
echo \\begin{code} >> $CURDIR/TEX;
echo \\label{AP:$col1} >> $CURDIR/TEX
echo \\tcaption{\$$col1\$}{\$$col1\$} >> $CURDIR/TEX
